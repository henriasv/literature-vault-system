...
export const bibtexTemplates: BibtexTemplate[] = [
  ...
  % citekey above is ignored — minted from author + year + title/DOI on save
    ...
    hint: "Non-standard @studentwork entry type for grading / commenting on student assignments. Custom fields: `course` / `assignmentid` / `studentid` / `group`. Set `type` to e.g. \"Term paper\", \"Project report\", \"Lab report\". These four custom fields are what a future \"read assignments\" mode would slice on.",
  },
  {
